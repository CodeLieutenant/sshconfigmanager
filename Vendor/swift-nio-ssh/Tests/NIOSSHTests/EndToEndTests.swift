...
class EndToEndTests: XCTestCase {
    ...
    func testServerDoesNotSendBanner() throws {
        class ClientHandshakeHandler: ChannelInboundHandler {
            ...
            func userInboundEventTriggered(context: ChannelHandlerContext, event: Any) {
                // Only the auth events settle this promise. A connection fires others —
                // `NIOSSHNegotiatedAlgorithms` at NEWKEYS, ahead of user auth — and
                // consuming one of those used to nil the promise without ever completing
                // it, hanging the `wait()` below forever.
    ...
    func testCorrectBannerReceived() throws {
        class ClientHandshakeHandler: ChannelInboundHandler {
            ...
            func userInboundEventTriggered(context: ChannelHandlerContext, event: Any) {
                // See the note in `testMissingBanner`: ignore events this test is not about,
                // rather than consuming the promise on the first one that arrives.
                guard let promise = self.promise else { return }
