    ...
    private func run(
        ...
        while clientMessage.value != nil || serverMessage.value != nil {
            ...
            serverLoop: while true {
                ...
                case .some(.event):
                    // The client loop above already ignores these. The server side lumped
                    // them in with the unsupported cases, which was harmless only while no
                    // event reached the server — `NIOSSHNegotiatedAlgorithms` at NEWKEYS
                    // does, and a `fatalError` here takes down the whole test process.
                    ()
                case .some(.forwardToMultiplexer), .some(.globalRequest), .some(.globalRequestResponse),
                    .some(.disconnect):
                    fatalError("Currently unsupported")
