...
/// `KexAlgorithms`/`HostKeyAlgorithms` override plumbing added by the sshconfigmanager
/// patch (see `SSHClientConfiguration.keyExchangeAlgorithmsOverride`/
/// `hostKeyAlgorithmsOverride`). Verifies the override actually reaches the offered
/// `SSH_MSG_KEXINIT` algorithm lists, that ordering follows the override (not the
/// default preference order), and that an unset override reproduces today's default
/// behavior exactly (no regression for every existing caller).
final class KeyExchangeAlgorithmOverrideTests: XCTestCase {
    private func makeClient(
        keyExchangeAlgorithmsOverride: [String]? = nil,
        hostKeyAlgorithmsOverride: [String]? = nil
    ) -> SSHKeyExchangeStateMachine {
        var configuration = SSHClientConfiguration(
            userAuthDelegate: ExplodingAuthDelegate(),
            serverAuthDelegate: AcceptAllHostKeysDelegate()
        )
        configuration.keyExchangeAlgorithmsOverride = keyExchangeAlgorithmsOverride
        configuration.hostKeyAlgorithmsOverride = hostKeyAlgorithmsOverride
        return SSHKeyExchangeStateMachine(
    ...
    func testUnsetOverrideOffersEveryAlgorithmUnchanged() {
        let client = self.makeClient()
        let message = client.createKeyExchangeMessage()
        XCTAssertEqual(message.keyExchangeAlgorithms, SSHKeyExchangeStateMachine.supportedKeyExchangeAlgorithms)
        XCTAssertEqual(message.serverHostKeyAlgorithms, SSHKeyExchangeStateMachine.supportedServerHostKeyAlgorithms)
    }

    func testOverrideRestrictsAndReordersKeyExchangeAlgorithms() {
        // Deliberately reversed order relative to the default preference list, and
        // missing one supported algorithm — both should be reflected in the offer.
        let reversed = Array(SSHKeyExchangeStateMachine.supportedKeyExchangeAlgorithms.dropFirst().reversed())
            .map(String.init)
        let client = self.makeClient(keyExchangeAlgorithmsOverride: reversed)
        let message = client.createKeyExchangeMessage()
        XCTAssertEqual(message.keyExchangeAlgorithms, reversed.map { Substring($0) })
        XCTAssertNotEqual(message.keyExchangeAlgorithms, SSHKeyExchangeStateMachine.supportedKeyExchangeAlgorithms)
    }

    func testOverrideRestrictsAndReordersHostKeyAlgorithms() {
        let reversed = Array(SSHKeyExchangeStateMachine.supportedServerHostKeyAlgorithms.dropFirst().reversed())
            .map(String.init)
        let client = self.makeClient(hostKeyAlgorithmsOverride: reversed)
        let message = client.createKeyExchangeMessage()
        XCTAssertEqual(message.serverHostKeyAlgorithms, reversed.map { Substring($0) })
        XCTAssertNotEqual(message.serverHostKeyAlgorithms, SSHKeyExchangeStateMachine.supportedServerHostKeyAlgorithms)
    }

    func testOverrideNamesNotSupportedByThisImplementationAreDropped() {
        let client = self.makeClient(keyExchangeAlgorithmsOverride: ["not-a-real-algorithm"])
        let message = client.createKeyExchangeMessage()
        XCTAssertEqual(message.keyExchangeAlgorithms, [])
    }

    /// An OpenSSH server hardened to ETM-only MACs (`hmac-sha2-*-etm@openssh.com`)
    /// shares no MAC name with us, because `supportedMacAlgorithms` fabricates a
    /// single `hmac-sha2-256` entry for AEAD schemes. The cipher is AES-GCM, which
    /// authenticates the packet itself, so OpenSSH skips MAC negotiation entirely —
    /// and so must we. Before the fix the whole handshake failed with
    /// `keyExchangeNegotiationFailure` and the client fell silent.
    func testAEADCipherIgnoresAnETMOnlyMACList() throws {
        var client = self.makeClient()
        let serverMessage = SSHMessage.KeyExchangeMessage(
            cookie: ByteBufferAllocator().buffer(capacity: 16),
            keyExchangeAlgorithms: ["curve25519-sha256", "ext-info-s", "kex-strict-s-v00@openssh.com"],
        ...
        let response = try client.handle(keyExchange: serverMessage)
    ...
    /// The post-quantum hybrid must be advertised ahead of every ECDH method, and must be
    /// what negotiation picks against an OpenSSH 9.9-style server.
    @available(macOS 26.0, iOS 26.0, tvOS 26.0, watchOS 26.0, macCatalyst 26.0, visionOS 26.0, *)
    func testPrefersTheMLKEMHybridWhenTheServerOffersIt() throws {
        var client = self.makeClient()
        ...
        let serverMessage = SSHMessage.KeyExchangeMessage(
            ...
            keyExchangeAlgorithms: [
                "mlkem768x25519-sha256", "sntrup761x25519-sha512@openssh.com", "curve25519-sha256",
            ...
            macAlgorithmsClientToServer: ["hmac-sha2-512-etm@openssh.com"],
            macAlgorithmsServerToClient: ["hmac-sha2-512-etm@openssh.com"],
            compressionAlgorithmsClientToServer: ["none"],
            compressionAlgorithmsServerToClient: ["none"],
            languagesClientToServer: [],
            languagesServerToClient: [],
        ...
        client.send(keyExchange: client.createKeyExchangeMessage())
        let response = try client.handle(keyExchange: serverMessage)
        guard case .some(.keyExchangeInit(let initMessage)) = response?.first else {
            XCTFail("Expected an ECDH key exchange init, got \(String(describing: response))")
            return
        }

        // C_INIT is the 1184-byte ML-KEM-768 encapsulation key followed by a 32-byte X25519
        // public key. Anything else and the server will reject the blob outright.
        XCTAssertEqual(initMessage.publicKey.readableBytes, 1184 + 32)
    }

    /// A server without the hybrid still negotiates, falling back to an ECDH method.
    func testFallsBackToECDHWhenTheServerHasNoPQMethod() throws {
        var client = self.makeClient()
        let serverMessage = SSHMessage.KeyExchangeMessage(
            cookie: ByteBufferAllocator().buffer(capacity: 16),
        ...
        XCTAssertEqual(initMessage.publicKey.readableBytes, 32)
    }
}
