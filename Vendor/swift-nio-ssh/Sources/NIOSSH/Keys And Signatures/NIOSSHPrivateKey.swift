...
public struct NIOSSHPrivateKey: Sendable {
    ...
    /// Wraps a custom private key type (see ``NIOSSHPrivateKeyProtocol``).
    /// NIOSSH patch (sshconfigmanager): CustomKeys.
...
extension NIOSSHPrivateKey {
    ...
    internal enum BackingKey {
        ...
        case custom(NIOSSHPrivateKeyProtocol)  // NIOSSH patch (sshconfigmanager): CustomKeys.
...
extension NIOSSHPrivateKey {
    func sign<DigestBytes: Digest>(digest: DigestBytes) throws -> NIOSSHSignature {
        ...
        case .custom:
            // NIOSSH patch (sshconfigmanager): CustomKeys are user-auth keys, never
            // host keys, so this digest (KEX exchange-hash) signing path is unused.
            // The custom-key protocol hashes its DataProtocol input, which would be
            // wrong for an already-computed digest — guard rather than mis-sign.
            preconditionFailure("custom keys are not supported as host keys")
    ...
    func sign(_ payload: UserAuthSignablePayload) throws -> NIOSSHSignature {
        ...
        case .ecdsaP521(let key):
            ...
            return NIOSSHSignature(backingSignature: .ecdsaP521(signature))
        case .custom(let key):  // NIOSSH patch (sshconfigmanager): CustomKeys.
            let signature = try key.signature(for: Array(payload.bytes.readableBytesView))
...
extension NIOSSHPrivateKey {
    ...
    public var publicKey: NIOSSHPublicKey {
        ...
        case .custom(let privateKey):  // NIOSSH patch (sshconfigmanager): CustomKeys.
