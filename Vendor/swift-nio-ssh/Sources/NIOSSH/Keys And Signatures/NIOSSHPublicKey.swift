...
extension NIOSSHPublicKey {
    ...
    internal func isValidSignature<DigestBytes: Digest>(_ signature: NIOSSHSignature, for digest: DigestBytes) -> Bool {
        ...
        case (.custom, .custom):
            // NIOSSH patch (sshconfigmanager): the digest overload verifies host-key
            // (KEX) signatures over a pre-computed digest. Custom keys are never
            // host keys here (a client only advertises ed25519/ECDSA host-key
            // algorithms), and the custom-key protocol would re-hash this digest, so
            // reject rather than verify incorrectly.
            return false
    ...
    internal func isValidSignature(_ signature: NIOSSHSignature, for bytes: ByteBuffer) -> Bool {
        ...
        case (.ecdsaP521(let key), .ecdsaP521(let sig)):
            return key.isValidSignature(sig, for: bytes.readableBytesView)
        case (.custom(let key), .custom(let sig)):  // NIOSSH patch (sshconfigmanager): CustomKeys.
            return key.isValidSignature(sig, for: Array(bytes.readableBytesView))
        case (.certified(let key), _):
    ...
    internal func isValidSignature(_ signature: NIOSSHSignature, for payload: UserAuthSignablePayload) -> Bool {
        ...
        case (.ecdsaP521(let key), .ecdsaP521(let sig)):
            return key.isValidSignature(sig, for: payload.bytes.readableBytesView)
        case (.custom(let key), .custom(let sig)):  // NIOSSH patch (sshconfigmanager): CustomKeys.
            return key.isValidSignature(sig, for: Array(payload.bytes.readableBytesView))
        case (.certified(let key), _):
...
extension NIOSSHPublicKey {
    ...
    internal enum BackingKey {
        ...
        case custom(NIOSSHPublicKeyProtocol)  // NIOSSH patch (sshconfigmanager): CustomKeys.
    ...
    internal var keyPrefix: String.UTF8View {
        ...
        case .custom(let publicKey):  // NIOSSH patch (sshconfigmanager): CustomKeys.
    ...
    /// "ssh-rsa" key blob with "rsa-sha2-256". NIOSSH patch (sshconfigmanager): CustomKeys + PR #3.
    ...
    internal static var knownAlgorithms: [String.UTF8View] {
        // NIOSSH patch (sshconfigmanager): CustomKeys + PR #3 — surface registered custom
        // key types, and their distinct userauth algorithm name (RFC 8332) when it differs.
...
extension NIOSSHPublicKey.BackingKey: Equatable {
    static func == (lhs: NIOSSHPublicKey.BackingKey, rhs: NIOSSHPublicKey.BackingKey) -> Bool {
        ...
        case (.ecdsaP521(let lhs), .ecdsaP521(let rhs)):
            return lhs.rawRepresentation == rhs.rawRepresentation
        case (.custom(let lhs), .custom(let rhs)):  // NIOSSH patch (sshconfigmanager): CustomKeys.
...
extension NIOSSHPublicKey.BackingKey: Hashable {
    func hash(into hasher: inout Hasher) {
        ...
        case .ecdsaP521(let pkey):
            hasher.combine(4)
            hasher.combine(pkey.rawRepresentation)
        case .custom(let pkey):  // NIOSSH patch (sshconfigmanager): CustomKeys.
            hasher.combine(6)
...
extension ByteBuffer {
    ...
    mutating func writeSSHHostKey(_ key: NIOSSHPublicKey) -> Int {
        ...
        case .custom(let key):  // NIOSSH patch (sshconfigmanager): CustomKeys.
    ...
    mutating func readPublicKeyWithoutPrefixForIdentifier<Bytes: Collection>(
        ...
        try self.rewindOnNilOrError { buffer in
            if keyIdentifierBytes.elementsEqual(NIOSSHPublicKey.ed25519PublicKeyPrefix) {
                ...
                // NIOSSH patch (sshconfigmanager): CustomKeys — try registered custom key types.
