...
extension NIOSSHSignature {
    ...
    internal enum BackingSignature: Sendable {
        ...
        case custom(NIOSSHSignatureProtocol)  // NIOSSH patch (sshconfigmanager): CustomKeys.
...
extension NIOSSHSignature.BackingSignature: Hashable {
    func hash(into hasher: inout Hasher) {
        ...
        case .ecdsaP521(let sig):
            hasher.combine(3)
            hasher.combine(sig.rawRepresentation)
        case .custom(let sig):  // NIOSSH patch (sshconfigmanager): CustomKeys.
            hasher.combine(4)
    ...
    mutating func readSSHSignature() throws -> NIOSSHSignature? {
        try self.rewindOnNilOrError { buffer in
            ...
            if bytesView.elementsEqual(NIOSSHSignature.ed25519SignaturePrefix) {
                ...
                // NIOSSH patch (sshconfigmanager): CustomKeys + PR #3 — try registered custom
                // signature types, matching any of their accepted wire prefixes (RFC 8332).
                for signature in NIOSSHPublicKey.customSignatures {
                    if signature.acceptedSignaturePrefixes.contains(where: { bytesView.elementsEqual($0.utf8) }) {
                        let parsed = try signature.read(from: &buffer)
                        return NIOSSHSignature(backingSignature: .custom(parsed))
                    }
                }
...
extension NIOSSHSignature {
    /// Parses an SSH wire-format signature blob — e.g. the signature an ssh-agent
    /// returns in a `SSH2_AGENT_SIGN_RESPONSE` (`string` algorithm name + `string`
    /// signature) — into a `NIOSSHSignature` for use with an
    /// ``NIOSSHUserAuthenticationOffer/Offer/externalKey(_:)`` offer.
    ///
    /// NIOSSH patch (sshconfigmanager): not in upstream swift-nio-ssh. See
    /// Vendor/PATCH.md.
    public init(sshWire blob: [UInt8]) throws {
