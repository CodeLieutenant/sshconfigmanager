...
protocol EllipticCurveKeyExchangeProtocol: _NIOSSHSendableMetatype {
    ...
    /// Throws only for methods whose client-side keypair can fail to materialise —
    /// `mlkem768x25519-sha256`, whose ML-KEM half comes from a registered backend. ECDH
    /// cannot fail here, so those implementations stay non-throwing and still conform.
...
extension EllipticCurveKeyExchange {
    ...
    private func generateKeys(
        ...
        //
        // ECDH folds K into the hash as an mpint; the derivation that follows is shared with
        // every other key exchange method and lives in `SSHKeyDerivation`.
        ...
        return SSHKeyDerivation.sessionKeys(
...
extension HashFunction {
    internal mutating func update(byte: UInt8) {
