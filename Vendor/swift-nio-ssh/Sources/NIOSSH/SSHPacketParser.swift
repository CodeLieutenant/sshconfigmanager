...
struct SSHPacketParser {
    ...
    mutating func nextPacket() throws -> SSHMessage? {
        ...
        case .cleartextWaitingForLength:
            if let length = self.buffer.getInteger(at: self.buffer.readerIndex, as: UInt32.self) {
                // Same unauthenticated header as the encrypted path, and this one runs
                // before key exchange, against a peer that has proved nothing at all.
                guard length <= Constants.maximumPacketLength else {
    ...
    private mutating func decryptLength(protection: NIOSSHTransportProtection) throws -> UInt32? {
        ...
        let length: UInt32 = self.buffer.getInteger(at: self.buffer.readerIndex)!
        // Bound the peer's length *before* the addition below. `length + macBytes`
        // overflows and traps for a length near `UInt32.max`, so an unchecked header let a
        // peer kill the process with a single packet.
        guard length <= Constants.maximumPacketLength else {
