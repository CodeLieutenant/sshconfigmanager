...
extension NIOSSHHandler {
    ...
    /// Sends a named global request with an empty, opaque payload and no way for the
    /// caller to interpret a reply beyond "did one arrive" — enough to implement an
    /// OpenSSH-style client keepalive (e.g. `keepalive@openssh.com`). A server that
    /// doesn't recognise the name still answers with SSH_MSG_REQUEST_FAILURE per
    /// RFC 4254 §4, which completes `promise` (as a failure) just as validly as a
    /// success would — either outcome proves the link is alive, which is all a
    /// keepalive probe needs. `sendGlobalRequestMessage` above already does the real
    /// work; this only exists because `SSHMessage`/`GlobalRequestMessage` are
    /// internal to this module, so the app has no other way to reach it.
    ///
    /// This function is **not** thread-safe: it may only be called from on the channel.
    /// NIOSSH patch (sshconfigmanager).
    public func sendGlobalRequest(named name: String, promise: EventLoopPromise<ByteBuffer?>? = nil) {
        let message = SSHMessage.GlobalRequestMessage(wantReply: true, type: .unknown(name, ByteBuffer()))
