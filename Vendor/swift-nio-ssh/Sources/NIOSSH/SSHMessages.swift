...
enum SSHMessage: Equatable {
    ...
    /// Raw bytes for wire ID 60, which is either SSH_MSG_USERAUTH_PK_OK or
    /// SSH_MSG_USERAUTH_INFO_REQUEST depending on which auth method is in flight.
    /// The UserAuthenticationStateMachine decodes this based on tracked context.
    /// NIOSSH patch (sshconfigmanager).
    case userAuth60Raw(ByteBuffer)
    /// SSH_MSG_USERAUTH_INFO_RESPONSE (61) — sent by the client to answer a
    /// keyboard-interactive challenge. NIOSSH patch (sshconfigmanager).
...
extension SSHMessage {
    ...
    struct UserAuthRequestMessage: Equatable {
        ...
        enum Method: Equatable {
            ...
            /// keyboard-interactive auth request (RFC 4256). Language-tag and
            /// submethods are always empty; the server drives the dialog.
            /// NIOSSH patch (sshconfigmanager).
    ...
    /// SSH_MSG_USERAUTH_INFO_REQUEST (RFC 4256 §3). Wire ID 60, same as PK_OK.
    /// Decoded from userAuth60Raw by UserAuthenticationStateMachine. NIOSSH patch (sshconfigmanager).
    struct UserAuthInfoRequestMessage: Equatable {
        ...
        static func == (lhs: Self, rhs: Self) -> Bool {
            ...
            lhs.prompts.count == rhs.prompts.count &&
            zip(lhs.prompts, rhs.prompts).allSatisfy { $0.text == $1.text && $0.echo == $1.echo }
        }
    }

    /// SSH_MSG_USERAUTH_INFO_RESPONSE (RFC 4256 §4). Wire ID 61. NIOSSH patch (sshconfigmanager).
...
extension ByteBuffer {
    ...
    mutating func readSSHMessage() throws -> SSHMessage? {
        try self.rewindOnNilOrError { `self` in
            ...
            case SSHMessage.UserAuthPKOKMessage.id:
                // ID 60 is SSH_MSG_USERAUTH_PK_OK *or* SSH_MSG_USERAUTH_INFO_REQUEST
                // (RFC 4256) depending on which auth method is in flight. Preserve the
                // raw bytes so the state machine can decode based on context.
                // NIOSSH patch (sshconfigmanager).
                guard let raw = self.readSlice(length: self.readableBytes) else {
                    return nil
                }
                return .userAuth60Raw(raw)
        ...
        try self.rewindOnNilOrError { `self` in
            ...
            case "publickey":
                ...
                if NIOSSHPublicKey.knownAlgorithms.contains(where: { $0.elementsEqual(algorithmName.readableBytesView) }
                    ...
                    // NIOSSH patch (sshconfigmanager): PR #3 — accept the RFC 8332 userauth
                    // algorithm name (e.g. "rsa-sha2-256") as well as the key blob prefix.
            ...
            case "keyboard-interactive":
                // RFC 4256 §3.1: read and discard language-tag and submethods. NIOSSH patch (sshconfigmanager).
                guard
    ...
    /// Decode SSH_MSG_USERAUTH_INFO_REQUEST from raw bytes (RFC 4256 §3.3).
    /// Called by UserAuthenticationStateMachine when keyboard-interactive is in flight.
    /// NIOSSH patch (sshconfigmanager).
    mutating func readUserAuthInfoRequestMessage() -> SSHMessage.UserAuthInfoRequestMessage? {
        self.rewindReaderOnNil { `self` in
            ...
            for _ in 0..<numPrompts {
                guard
                    let text = self.readSSHStringAsString(),
                    let echo = self.readSSHBoolean()
                else {
                    return nil
                }
                prompts.append((text: text, echo: echo))
            }
    ...
    mutating func readUserAuthPKOKMessage() throws -> SSHMessage.UserAuthPKOKMessage? {
        try self.rewindOnNilOrError { `self` in
            ...
            // NIOSSH patch (sshconfigmanager): PR #3 — also accept the RFC 8332 userauth name.
...
extension ByteBuffer {
    ...
    mutating func writeSSHMessage(_ message: SSHMessage) -> Int {
        ...
        case .userAuth60Raw:
            // Inbound-only; never serialised. NIOSSH patch (sshconfigmanager).
            preconditionFailure("userAuth60Raw is inbound-only and cannot be serialized")
    ...
    mutating func writeUserAuthRequestMessage(_ message: SSHMessage.UserAuthRequestMessage) -> Int {
        ...
        case .publicKey(.known(key: let key, signature: let signature)):
            ...
            // NIOSSH patch (sshconfigmanager): PR #3 — advertise the RFC 8332 userauth
            // algorithm name (e.g. "rsa-sha2-256"), which equals keyPrefix for bundled keys.
        ...
        case .keyboardInteractive:
            // RFC 4256 §3.1: method + empty language-tag + empty submethods. NIOSSH patch (sshconfigmanager).
    ...
    /// Encode SSH_MSG_USERAUTH_INFO_RESPONSE (RFC 4256 §3.4). NIOSSH patch (sshconfigmanager).
    @discardableResult
