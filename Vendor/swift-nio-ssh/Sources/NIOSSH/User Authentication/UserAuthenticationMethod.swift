...
public struct NIOSSHAvailableUserAuthenticationMethods: OptionSet, Sendable {
    ...
    /// Keyboard-interactive authentication is acceptable (RFC 4256).
...
extension NIOSSHUserAuthenticationOffer {
    ...
    public enum Offer: Sendable {
        ...
        /// The client would like to perform public-key authentication where the
        /// signature is produced by an external signer (e.g. an ssh-agent) that
        /// holds the private key. NIOSSH builds the signable payload and hands it
        /// to `ExternalKey.sign`; the private key never enters this process.
        ///
        /// NIOSSH patch (sshconfigmanager): not in upstream swift-nio-ssh. See
        /// Vendor/PATCH.md.
        case externalKey(ExternalKey)
        ...
        /// The server drives the dialog via SSH_MSG_USERAUTH_INFO_REQUEST prompts; responses
        /// are provided by NIOSSHKeyboardInteractiveDelegate. NIOSSH patch (sshconfigmanager).
...
extension NIOSSHUserAuthenticationOffer.Offer {
    ...
    /// Information for public-key authentication where signing is delegated to an
    /// external signer (e.g. an ssh-agent). NIOSSH patch (sshconfigmanager).
    public struct ExternalKey: Sendable {
        /// The public key sent to the server.
        public var publicKey: NIOSSHPublicKey

        /// Produces the SSH-wire signature over the exact bytes NIOSSH must sign
        /// for this auth attempt. Called once, off the event loop.
        public var sign: @Sendable (ByteBuffer) async throws -> NIOSSHSignature

        public init(publicKey: NIOSSHPublicKey,
                    sign: @escaping @Sendable (ByteBuffer) async throws -> NIOSSHSignature) {
            self.publicKey = publicKey
            self.sign = sign
        }
    }
...
extension SSHMessage.UserAuthRequestMessage {
    init(request: NIOSSHUserAuthenticationOffer, sessionID: ByteBuffer) throws {
        ...
        case .externalKey:
            // External signing is asynchronous; the state machine builds this
            // message via the async path (UserAuthenticationStateMachine), never
            // through this synchronous initializer. NIOSSH patch (sshconfigmanager).
            preconditionFailure("externalKey offers are signed via the async path")
        case .password(let passwordRequest):
            self.method = .password(passwordRequest.password)
        case .hostBased:
            fatalError("Unsupported")
        case .keyboardInteractive:
            // RFC 4256: no session-ID signing required; the dialog is driven by the server.
            // NIOSSH patch (sshconfigmanager).
