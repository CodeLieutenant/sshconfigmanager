...
public protocol NIOSSHTransportProtection: AnyObject {
    ...
    /// passed to `source`, with the same `sequenceNumber`. Thus some implementations will be able
    /// to avoid storing state for partial packet decryption.
    ///
    /// `sequenceNumber` is not in upstream swift-nio-ssh, which passes it only to
    /// `decryptAndVerifyRemainingPacket`. `chacha20-poly1305@openssh.com` encrypts the length
    /// field under a second key whose nonce *is* the sequence number, so without it here that
    /// scheme cannot be implemented without shadowing the parser's sequence counter — and the
    /// counter runs for the life of the connection, so a scheme installed at rekey has no way to
    /// learn where it currently is.
