...
extension ChildChannelStateMachine {
    ...
    mutating func sendChannelWindowAdjust(_ message: SSHMessage.ChannelWindowAdjustMessage) throws {
        ...
        case .closedLocally, .closedRemotely, .closed:
            // NIOSSH patch (sshconfigmanager): PR #2 — a read can be delivered from the
            // multiplexer's pending-read queue after the channel was closed; the resulting
            // window adjust must not crash the process. Surface it as a protocol error.
            throw NIOSSHError.protocolViolation(
                protocolName: "channel",
