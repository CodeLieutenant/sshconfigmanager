        ...
        mutating func processInboundMessage(
            ...
            case .keyExchange(var state):
                do {
                    ...
                    case .keyExchangeReply(let message):
                        ...
                        return result
                    case .newKeys:
                        let negotiated = try state.receiveNewKeysMessage()
                        self = .receivedNewKeys(.init(keyExchangeState: state, loop: loop))
                        return negotiated.map { .event($0) } ?? .noMessage
                    case .disconnect:
                        self = .receivedDisconnect(state.role)
                        return .disconnect
                    case .ignore, .debug:
            ...
            case .userAuthentication(var state):
                do {
                    ...
                    case .userAuth60Raw(let buffer):
                        // NIOSSH patch (sshconfigmanager): ID 60 decoded as INFO_REQUEST or PK_OK.
                        let result = try state.receiveUserAuth60Raw(buffer)
                        self = .userAuthentication(state)
                        return result
