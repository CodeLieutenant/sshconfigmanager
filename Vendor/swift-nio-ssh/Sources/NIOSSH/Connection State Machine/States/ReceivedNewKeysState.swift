...
extension SSHConnectionStateMachine {
    ...
    struct ReceivedNewKeysState {
        ...
        init(
            ...
            // Inject keyboard-interactive delegate from client config. NIOSSH patch (sshconfigmanager).
            if case .client(let config) = self.role {
                self.userAuthStateMachine.keyboardInteractiveDelegate = config.keyboardInteractiveDelegate
            }
        }
