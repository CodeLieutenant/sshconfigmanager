...
extension AcceptsUserAuthMessages {
    ...
    /// Handle wire ID 60 (SSH_MSG_USERAUTH_PK_OK or SSH_MSG_USERAUTH_INFO_REQUEST).
    /// NIOSSH patch (sshconfigmanager).
    mutating func receiveUserAuth60Raw(
        _ buffer: ByteBuffer
    ) throws -> SSHConnectionStateMachine.StateMachineInboundProcessResult {
        let responseFuture = try self.userAuthStateMachine.receiveUserAuth60Raw(buffer)
        guard let responseFuture else { return .noMessage }
        return .possibleFutureMessage(
            responseFuture.map { SSHMultiMessage(.userAuthInfoResponse($0)) }
        )
    }
