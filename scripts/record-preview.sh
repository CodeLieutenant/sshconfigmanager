#!/usr/bin/env bash
#
# record-preview.sh — App Store preview video: full-feature app overview.
#
# Drives AppPreviewDemoUITests through a ~28 s cinematic walkthrough of the
# whole app: sidebar groups → host detail → add-setting catalog → raw editor
# IntelliSense → tunnels → keys → SSH agent → known hosts → version history.
...
#
# Usage:
#   ./scripts/record-preview.sh
#   RECORD_SECONDS=35 ./scripts/record-preview.sh
#
# Output:
#   build/preview/app-preview.mov          raw window capture (gitignored)
#   build/preview/app-preview.mp4          App-Store-ready H.264 (gitignored)
#   fastlane/previews/en-US/app-preview.mp4  versioned copy (committed)
#
...
RAW="$OUT_DIR/app-preview.mov"
OUT="$OUT_DIR/app-preview.mp4"
RECORD_SECONDS="${RECORD_SECONDS:-33}"
TARGET="${TARGET:-1920x1080}"
LOG="$OUT_DIR/app-preview-test.log"
FADE_IN="${FADE_IN:-0.4}"
...
echo "==> Building AppPreviewDemoUITests (signed)"
...
echo "==> Launching full-feature preview walkthrough (AppPreviewDemoUITests)"
xcodebuild test-without-building \
  -project "$PROJECT" -scheme "$SCHEME" \
  -destination 'platform=macOS' \
  -only-testing:sshconfigmanagerUITests/AppPreviewDemoUITests \
  >"$LOG" 2>&1 &
TEST_PID=$!
