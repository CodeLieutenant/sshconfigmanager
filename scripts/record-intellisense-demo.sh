#!/usr/bin/env bash
#
# record-intellisense-demo.sh — App Store preview video: IntelliSense showcase.
#
# Drives IntelliSenseDemoUITests through a cinematic walkthrough of all
# completion kinds: snippet (LocalForward), keyword (Compression), enum
# (yes/no), ProxyJump + host alias, and cipher algorithm list. Records ONLY
# the app window via screencapture -l<winid> (no desktop, no overlay), then
# encodes to H.264 at the App Store spec (1920×1080, 30fps, CRF 18, ≤29s).
#
# Usage:
#   ./scripts/record-intellisense-demo.sh
#   RECORD_SECONDS=35 TARGET=1920x1080 ./scripts/record-intellisense-demo.sh
#
# Output:
#   build/preview/intellisense-demo.mov          raw window capture (gitignored)
#   build/preview/intellisense-demo.mp4          App-Store-ready H.264 (gitignored)
#   fastlane/previews/en-US/intellisense-demo.mp4  versioned copy (committed)
#
...
RAW="$OUT_DIR/intellisense-demo.mov"
OUT="$OUT_DIR/intellisense-demo.mp4"
RECORD_SECONDS="${RECORD_SECONDS:-33}"
TARGET="${TARGET:-1920x1080}"
LOG="$OUT_DIR/intellisense-demo-test.log"
FADE_IN="${FADE_IN:-0.4}"
...
echo "==> Building IntelliSenseDemoUITests (signed)"
...
echo "==> Launching IntelliSense demo walkthrough (IntelliSenseDemoUITests)"
xcodebuild test-without-building \
  -project "$PROJECT" -scheme "$SCHEME" \
  -destination 'platform=macOS' \
  -only-testing:sshconfigmanagerUITests/IntelliSenseDemoUITests \
  >"$LOG" 2>&1 &
TEST_PID=$!
...
FINAL_DUR="$(ffprobe -v error -show_entries format=duration -of csv=p=0 "$OUT" 2>/dev/null || echo '?')"
echo "Done:"
